...
// Убираем X-Frame-Options полностью - пусть браузер сам решает
header('Access-Control-Allow-Origin: *');
header('Access-Control-Allow-Methods: GET, POST, OPTIONS');
header('Access-Control-Allow-Headers: Content-Type, Authorization');
// Добавляем CSP заголовок для разрешения iframe
...
echo "<!-- DEBUG: Autoloader exists -->\n";

try {
    require_once __DIR__ . '/../vendor/autoload.php';
    echo "<!-- DEBUG: Autoloader loaded -->\n";
} catch (Exception $e) {
    die('ERROR: Autoloader failed: ' . $e->getMessage());
}

// Если нет параметров - показываем тестовый iframe
if (empty($_REQUEST)) {
    echo "<!-- DEBUG: No parameters, showing test iframe -->\n";
    $targetUrl = 'https://dimpin-app.store/apps/led-calculator/';
    echo '<!DOCTYPE html><html><head><title>LED калькулятор (TEST)</title></head><body>';
    echo '<h2>TEST MODE - No Parameters</h2>';
    echo '<iframe src="' . htmlspecialchars($targetUrl) . '" width="100%" height="600px" frameborder="0" allow="scripts, forms" sandbox="allow-scripts allow-forms allow-same-origin"></iframe>';
    echo '</body></html>';
    exit;
}

echo "<!-- DEBUG: Has parameters, continuing with original logic -->\n";
...
echo "<!-- DEBUG: isInstallation=$isInstallation, isPlacementCall=$isPlacementCall -->\n";

// Если это установка - перенаправляем на install.php
if ($isInstallation) {
    echo "<!-- DEBUG: Redirecting to install.php -->\n";
...
if (!$isPlacementCall) {
    echo "<!-- DEBUG: Not a placement call, showing info page -->\n";
...
echo "<!-- DEBUG: This is a placement call, proceeding with SDK logic -->\n";
...
try {
    ...
    echo "<!-- DEBUG: Credentials created -->\n";
    ...
    echo "<!-- DEBUG: SDK Core initialized -->\n";

    // --- ЗАПУСК КАЛЬКУЛЯТОРА ---
    // В версии 1.5 используем данные из $_REQUEST напрямую
    $dealId = $_REQUEST['ID'] ?? $_REQUEST['id'] ?? null;
    if (empty($dealId)) {
        // Пытаемся получить из placement информации, если доступно
        $dealId = 'demo'; // fallback для демо
    }
    
    // Упрощаем - передаем доступные данные напрямую из $_REQUEST
    $queryParams = http_build_query([
        'dealId' => $dealId, 
        'userId' => 'current', // fallback значение
        'domain' => $_REQUEST['DOMAIN'] ?? 'unknown.bitrix24.ru', 
        'authId' => $_REQUEST['AUTH_ID'] ?? 'demo_auth', 
        'memberId' => $_REQUEST['member_id'] ?? 'demo_member'
    ]);
    
    echo "<!-- DEBUG: Query params created: $queryParams -->\n";
    ...
    echo '<!DOCTYPE html><html><head><title>LED калькулятор (Fallback)</title></head><body>';
